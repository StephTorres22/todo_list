...
export const newTaskButtonArray = Array.from(
  document.querySelectorAll(".newTaskButton")
);

export const taskListDivArray = Array.from(
  document.querySelectorAll(".projectArrayDiv")
);

export const expandButtonArray = Array.from(
  document.querySelectorAll(".expandButton")
);

export const removeProjectButtonArray = Array.from(
  document.querySelectorAll(".removeProjectButton")
);

function addListenerToRemoveButton() {
  removeProjectButtonArray.forEach((button, index) =>
...
domModule.closeProjectModalButton.addEventListener(
  "click",
...
function projectFormValidation() {
  ...
  if (
    domModule.projectTitle.value == "" ||
    domModule.projectTitle.value == null
  ) {
    ...
    domModule.projectDescription.value == "" ||
    domModule.projectDescription.value == null
  ) {
...
domModule.submitTaskButton.addEventListener("click", (e) => {
  if (
    domModule.inputDiv.contains(
      domModule.createDropDownList.projectDropDownList
    )
  ) {
    ...
    addTaskViaProjectCard(
      domModule.submitTaskButton.getAttribute("id"),
      getRadioGroupValue()
    );
    domModule.closeTaskModal();
    e.preventDefault();
...
domModule.closeTaskModalButton.addEventListener(
  "click",
  domModule.closeTaskModal
);
...
function checkTaskFormInputs(index) {
  ...
  if (taskTitles.includes(domModule.taskTitle.value)) {
    alert(
      "A task with that name already exists within this project, please choose another name."
    );
    return false;
  }
  ...
  if (
    domModule.taskDueDate.value == null ||
    domModule.taskDueDate.value == undefined ||
...
export function changeTaskSubmitID() {
  newTaskButtonArray.forEach((button, index) =>
    button.addEventListener("click", (e) => {
...
function repopulateProjectListFromLocalStorage(array) {
  if (localStorage.length !== 0) {
    ...
    array.forEach((project, pindex) => {
      let existingProject = new Project(
        `${project.title}`,
        `${project.description}`,
        `${project.dateCreated}`,
      ...
      project.projectArray.forEach((task, index) => {
        let existingTask = new Task(
          `${project.projectArray[index].title}`,
...
window.deserialiseProjectListFromLocalStorage =
  deserialiseProjectListFromLocalStorage;
window.repopulateProjectListFromLocalStorage =
  repopulateProjectListFromLocalStorage;
